...
module.exports.downLoad = function (url, type, api, event) {
    ...
    this.getLink(url).then(res => {
        ...
        axios({
            ...
            if (fs.statSync(path).size / 3000 / 3000 > 50) return api.sendMessage("File quá lớn, không thể gửi", event.threadID, () => fs.unlinkSync(path), event.messageID);
            api.sendMessage({
...
module.exports.checkLink = function (url) {
    ...
    var media = ['facebook', 'douyin', 'youtube', 'youtu', 'twitter', 'instagram', 'kuaishou', 'fb']
